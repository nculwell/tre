...
void run_editor(TRE_RT *runtime) {
  while (1) {
    int ch = read_char();
    handle_input(runtime, ch);
  }
}

void handle_input(TRE_RT *runtime, int ch)
{
  if (ch == 'q' || ch == 'Q')
  {
    exit(0);
...
int main(int argc, char *argv[])
{
  ...
  TRE_RT *rt = TRE_RT_init(opts); /* TODO: add in runtime (runs init scripts) */
